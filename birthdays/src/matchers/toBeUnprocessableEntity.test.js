...
describe('toBeUnprocessableEntity', () => {
	...
	it('throws if the status is not 422', () => {
		...
		expect(() => expect(response).toBeUnprocessableEntity()).toThrowError();
	});

	it('does not throw if the status is 422', () => {
		const response = fail(422);
		expect(() => expect(response).toBeUnprocessableEntity()).not.toThrowError();
	});
	...
	describe('not', () => {
		it('returns a message if the status is 422 with the same body', () => {
			...
			).toThrowError(/Expected non-422 status code but got 422/);
		});
		...
		it('returns a negated message for a non-422 status with no body', () => {
			const response = fail(422);
			expect(() => expect(response).not.toBeUnprocessableEntity()).toThrowError(
