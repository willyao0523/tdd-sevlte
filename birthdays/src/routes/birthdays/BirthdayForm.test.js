...
describe('BirthdayForm', () => {
	...
	describe('id field', () => {
		it('contains a hidden field for the id if an id is given', () => {
			...
			expect(document.forms.birthday.elements.id.value).toEqual('123');
		});

		it('does not include the id field if no id is present', () => {
			render(BirthdayForm);
			expect(document.forms.birthday.elements.id).not.toBeDefined();
		});
	});
});
...
describe('validation errors', () => {
	...
	it('keeps the previews dob value when an error occurs', () => {
		render(BirthdayForm, {
			form: {
				dob: '1994-01-01',
				error: 'Some awful error message'
		...
		expect(screen.queryByLabelText('Date of birth')).toHaveValue('1994-01-01');
	});
});
