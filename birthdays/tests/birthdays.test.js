...
test('does not save birthday if there are validation errors', async ({
	page
}) => {
	await page.goto('/birthdays');
	...
	await page.getByRole('button', { name: 'Save' }).click();
	await expect(page.getByText('Demeter')).not.toBeVisible();
	await expect(
		page.getByText('Please provide a date of birth in the YYYY-MM-DD format.')
	).toBeVisible();
});

test('edit a birthday', async ({ page }) => {
	const birthdayListPage = new BirthdayListPage(page);
	await birthdayListPage.goto('/birthdays');
	...
	await expect(birthdayListPage.entryFor('Ares')).not.toContainText(
		'1985-01-01'
	);
